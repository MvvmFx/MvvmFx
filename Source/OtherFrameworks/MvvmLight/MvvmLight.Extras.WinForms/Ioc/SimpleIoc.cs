...
#if !NETFX_CORE
using MvvmFx.MvvmLight.Properties;
#endif

namespace MvvmFx.MvvmLight.Ioc
{
    /// <summary>
    ...
    public class SimpleIoc : ISimpleIoc
    {
            ...
            lock (_syncLock)
            {
                    ...
                    && _factories[classType].ContainsKey(_defaultKey))
                {
                    if (!_constructorInfos.ContainsKey(classType))
                    {
                        ...
                        // for this class.
                        throw new InvalidOperationException(
                            string.Format(CultureInfo.CurrentCulture, "Class {0} is already registered.", classType));
                    }
            ...
            where TClass : class
        {
            ...
            lock (_syncLock)
            {
                ...
                {
                    throw new InvalidOperationException(
                        string.Format(CultureInfo.CurrentCulture, "There is already a factory registered for {0}.", classType.FullName));
                }
        ...
        private void DoRegister<TClass>(Type classType, Func<TClass> factory, string key)
        {
            if (_factories.ContainsKey(classType))
            {
                if (_factories[classType].ContainsKey(key))
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            string.Format(CultureInfo.CurrentCulture, "Class {0} is already registered.", classType.FullName));
                    }

                    throw new InvalidOperationException(
                        string.Format(CultureInfo.CurrentCulture, "Class {0} is already registered with key {1}.", classType.FullName, key));
                }
        ...
        private ConstructorInfo GetConstructorInfo(Type serviceType)
        {
            ...
            if (constructorInfos.Length > 1)
            {
                ...
                {
                    throw new ActivationException(
                        "Cannot build instance: Multiple constructors found but none marked with Preferred Constructor.");
                }
